 ...
 * 👑 rank.js: 닉네임 설정 및 랭킹 시스템 통합 엔진
 * 수정 사항: 
 * 1. [Critical] 데이터 전송 방식을 JSON -> URLSearchParams로 변경 (구글 시트 호환성 극대화)
 * 2. 닉네임 20자 제한 및 중복 저장 방지 로직 유지
 * 3. 저장 실패 시 콘솔에 명확한 에러 로그 출력
 */
...
/**
 * 4. 💾 점수 저장 (연결성 강화 버전: Form Data 방식)
 */
async function saveScoreToDB(score) {
    if (!window.RANKING_SERVER_URL || isSaving) return; 
    isSaving = true;

    // 🚀 [핵심 수정] JSON 대신 URLSearchParams 사용 (Form Data 방식)
    const formData = new URLSearchParams();
    formData.append('action', 'save');
    formData.append('name', userTempNickname);
    formData.append('score', score);
    formData.append('chapter', currentChapter.name || currentChapter);
    ...
    try {
        await fetch(window.RANKING_SERVER_URL, {
            ...
            body: formData // 폼 데이터 객체 전송
        });
        
        console.log("✅ 점수 전송 신호 보냄 (Form Data)");
        setTimeout(updateRankingUI, 1500); // 1.5초 후 랭킹 갱신

    } catch (e) { 
        console.error("❌ 저장 통신 실패:", e);
    } finally { 
        isSaving = false; 
    }
}
